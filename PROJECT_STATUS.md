...
components/
  Hero.tsx                Hero condivisa su TUTTE le pagine pubbliche (logo, titolo, sottotitolo, LiveBadge, social icons). Titolo (`h1`): `text-shadow` leggero inline (`[text-shadow:0_2px_8px_rgba(0,0,0,0.5)]`, per la leggibilità su sfondi mossi) e `tracking-normal` invece di `tracking-tight` (spaziatura tra le lettere leggermente aumentata, su richiesta esplicita — nessuna delle due è scoped a un tema, vale ovunque). Sezione hero: `pb-10 pt-8` invece di `pb-12 pt-10` (altezza complessiva ridotta leggermente, su richiesta esplicita) — attenzione: il tema Onda d'Urto (`impact`) sovrascrive `padding-bottom` a `34px` fisso via CSS, quindi non eredita questa riduzione, per via del taglio diagonale che gli serve sotto. Sfondo `home_bg_url`: la maschera radiale (vignette con bordi sfumati) che prima era inline nello `style` ora vive nella classe `.hero-bg-mask` (globals.css), disattivata di default (mobile-first) e riattivata solo da `sm:` (≥640px) in su — su smartphone la foto copre l'intera larghezza dello schermo edge-to-edge, senza il vignette che ai lati stretti lasciava intravedere lo sfondo della pagina dando l'impressione che l'immagine non si adattasse alla larghezza; da tablet in su il vignette torna identico a prima. Lo sfondo separato `header_bg_url` (solo su Home, `app/page.tsx`) non ha mai avuto maschera — è un semplice `background-size: cover` — quindi non necessitava di questa correzione.
