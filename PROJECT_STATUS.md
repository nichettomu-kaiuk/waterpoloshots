...
components/
  Hero.tsx                Hero condivisa su TUTTE le pagine pubbliche (logo, titolo, sottotitolo, LiveBadge, social icons). Titolo (`h1`): `text-shadow` leggero inline (`[text-shadow:0_2px_8px_rgba(0,0,0,0.5)]`, per la leggibilità su sfondi mossi) e `tracking-normal` invece di `tracking-tight` (spaziatura tra le lettere leggermente aumentata, su richiesta esplicita — nessuna delle due è scoped a un tema, vale ovunque). Sezione hero: `pb-10 pt-8` invece di `pb-12 pt-10` (altezza complessiva ridotta leggermente, su richiesta esplicita) — attenzione: il tema Onda d'Urto (`impact`) sovrascrive `padding-bottom` a `34px` fisso via CSS, quindi non eredita questa riduzione, per via del taglio diagonale che gli serve sotto. Sfondo `home_bg_url`: dimensioni e maschera ora vivono nella classe `.hero-bg` (globals.css), responsive via media query invece che inline nello `style`. Prima versione del fix: solo maschera disattivata su smartphone con `background-size: cover` invariato — non bastava, la foto restava comunque ritagliata per riempire un riquadro molto più largo che alto. Versione attuale (su richiesta esplicita, "non tagliata ma al più ridotta"): su smartphone `background-size: contain` invece di `cover` (la foto non viene mai ritagliata, solo rimpicciolita quanto basta per starci intera), ancorata in alto e centrata in orizzontale, senza vignette (su un'immagine "contain" il fade radiale taglierebbe via pezzi di foto invece di sfumarne solo i bordi); da tablet in su torna il comportamento originale invariato: `cover` + vignette radiale sui bordi. Nota: `contain` garantisce la larghezza piena solo per foto proporzionalmente "larghe" (tipico per un banner header, es. 16:9 o più panoramiche) — una foto molto verticale verrebbe invece centrata con bande laterali, per non tagliarla mai. Lo sfondo separato `header_bg_url` (solo su Home, `app/page.tsx`) non ha mai avuto maschera — è un semplice `background-size: cover` — e non è stato toccato: la richiesta era scoped esplicitamente a "header/hero", cioè questo componente condiviso.
